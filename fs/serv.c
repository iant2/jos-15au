...
serve_read(envid_t envid, union Fsipc *ipc)
{
	...
	size_t req_n = ipc->read.req_n;

	if (req_n > PGSIZE)
		req_n = PGSIZE;

	bytesRead = file_read(o->o_file, ret->ret_buf, req_n, offset);
	if (bytesRead < 0)
		return r;

	o->o_fd->fd_offset += bytesRead;
...
serve_write(envid_t envid, struct Fsreq_write *req)
{
	...
	r = file_write(o->o_file, req->req_buf, req->req_n, offset);
