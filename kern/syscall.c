...
sys_exofork(void)
{
	...
	struct Env * child;
	int alloc = env_alloc(&child, curenv->env_id);
...
sys_page_alloc(envid_t envid, void *va, int perm)
{
	...
	//cprintf("sys_page_alloc: if(!(perm & PTE_U) || !(perm & PTE_P) || (perm & ~PTE_SYSCALL)) \n");
	...
	//cprintf("sys_page_alloc: return page_insert(env->env_pgdir, pp, va, perm); \n");
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	if((uintptr_t)dstva > UTOP)
		return -E_INVAL;
	if((uintptr_t)dstva != ROUNDDOWN((uintptr_t)dstva, PGSIZE))
		return -E_INVAL;
	...
	struct Env * srcenv;
	int srcres = envid2env(srcenvid, &srcenv, 1);
	if (srcres < 0)
		return -E_BAD_ENV;

	struct Env * dstenv;
	int dstres = envid2env(dstenvid, &dstenv, 1);
	if (dstres < 0)
		return -E_BAD_ENV;

	struct PageInfo * pp;
	pte_t * page_table_entry;
	pp = page_lookup(srcenv->env_pgdir, srcva, &page_table_entry);
	if (!pp)
		return -E_INVAL;
...
//		or the caller doesn't have permission to change envid.
//	DONE-E_INVAL if va >= UTOP, or va is not page-aligned.
static int
...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
	...
	if((uintptr_t)srcva < UTOP && (uintptr_t)env->env_ipc_dstva < UTOP){
		...
		//	-E_NO_MEM if there's not enough memory to map srcva in envid's
		//		address space.
		if((res = page_insert(env->env_pgdir, pp, env->env_ipc_dstva, perm)) < 0){
			return res;
		}
	}
	...
	//    env_ipc_value is set to the 'value' parameter;
	//    env_ipc_perm is set to 'perm' if a page was transferred, 0 otherwise
	//cprintf("send succeeds \n");
	
	env->env_ipc_recving = 0; //is set to 0 to block future sends;
	env->env_ipc_from = curenv->env_id; //is set to the sending envid;
	env->env_ipc_value = value; //is set to the 'value' parameter;
	
	if ((uintptr_t)srcva < UTOP){
		env->env_ipc_perm = perm;
	} else {
		env->env_ipc_perm = 0;
	} //is set to 'perm' if a page was transferred, 0 otherwise
...
static int sys_env_priority(int priority){
...
syscall(uint32_t syscallno, uint32_t a1, uint32_t a2, uint32_t a3, uint32_t a4, uint32_t a5)
{
	...
	*/
	...
	switch (syscallno) {
        ...
        case SYS_yield:
			// Lab 4:  Don’t forget to modify syscall() to dispatch sys_yield().
			sys_yield();
			break;
